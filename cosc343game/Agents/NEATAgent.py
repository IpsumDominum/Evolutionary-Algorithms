...
def newGeneration(old_population):
    if(GameVars.LOADED==False):
        GameVars.LOADED = True
        #for i in range(34):
        #    old_population[i].save_pop("4",i)
        #for n,pop in enumerate(old_population):
        #    pop.load("best",n)
    #unbiased = np.array([unbiased_fitness(creature) for creature in old_population])
    #return (old_population,unbiased.mean())
    pop_fitness = np.array([get_fitness_eval(creature) for creature in old_population])
    #if(np.mean(pop_fitness)>GameVars.best_pop):
    #    GameVars.best_pop = np.mean(pop_fitness)
    #    for i in range(34):
    #       old_population[i].save_pop("best",i)
    with open("neatfitWANN.txt","a") as file:
        file.write(str(np.mean(pop_fitness))+"\n")
